...
async def _():
    ...
    await loop.run_in_executor(None, mongo_db.fans_count.insert, insert_data)
    last_hour = now - datetime.timedelta(hours=1)
    filter_ = {'t': last_hour.strftime("%Y-%m-%d %H")}
    check_last_hour = await loop.run_in_executor(None, mongo_db.fans_count.find_one, filter_)
