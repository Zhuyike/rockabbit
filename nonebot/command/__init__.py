...
def _find_command(name: Union[str, CommandName_T]) -> Optional[Command]:
    cmd_name = (name,) if isinstance(name, str) else name
    if '[CQ:image,' in cmd_name[0]:
        return _registry.get('cq_image_tongji____xxxx')
    if '[CQ:rich,content={"detail' in cmd_name[0]:
        return _registry.get('zhuanfazhuanfazhuanfa')
    if '[CQ:rich,content={"news' in cmd_name[0]:
        return _registry.get('zhuanfazhuanfazhuanfa_2')
    if '[CQ:rich,text=' in cmd_name[0]:
        return _registry.get('zhuanfazhuanfazhuanfa_hd')
    # if '高仿' in cmd_name[0]:
    #     return _registry.get('fuck高仿')
    if not cmd_name:
        return None
    cmd_tree = _registry
